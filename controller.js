...
exports.note = function (req, res) {
  ...
  let startLimit =
    queryParams.page > 1 ? queryParams.page * limits - limits : 0;

  connection.query(
    // Count Total data on Data Note
    ...
    function (err, rows) {
      ...
      if (
        ...
        // If All Undefined, Run this Code
        connection.query(
...
exports.category = function (req, res) {
  connection.query("SELECT * FROM tb_category", function (err, rows, field) {
    if (err) {
      console.log(err);
    } else {
    ...
    function (err, rows, field) {
      if (err) {
        ...
        return res.send({
          ...
          data: rows,
          message:
            "Data Category has been Created (データCategoryが作成されました)",
        });
      }
    }
  );
    ...
    function (err, rows, field) {
      if (err) {
        ...
        return res.send({
          ...
          data: rows,
          message:
            "Data Category has been Edited (データCategoryが編集されました)",
        });
      }
    }
  );
...
exports.deleteNote = function (req, res) {
  let noteID = req.body.note_id;
  connection.query(`DELETE FROM tb_note WHERE id_note=?`, [noteID], function (
    err,
    rows,
    field
  ) {
    ...
    function (err, rows, field) {
      if (err) {
        ...
        return res.send({
          ...
          data: rows,
          message:
            "Data Category has been Deleted (データCategoryは削除されました)",
        });
      }
    }
  );
